...
def migrate_vms(
    ...
    with run_migration(**run_migration_kwargs, skip_teardown=skip_teardown) as migration_plan:
...
def run_migration(
    ...
    with create_and_store_resource(skip_teardown=skip_teardown, **plan_kwargs) as _plan:
        plan = cast(Plan, _plan)
        try:
        ...
        with create_and_store_resource(
            ...
            # Migration is created and will be cleaned up automatically
            # We don't use the migration object, but it's created and tracked
            pass

        # CRITICAL: yield instead of return - keeps context alive
        yield plan
    # Plan and Migration cleaned up here AFTER caller finishes using them
...
def get_storage_migration_map(
    ...
    with create_and_store_resource(
        ...
        yield cast(StorageMap, _storage_map)
...
def get_network_migration_map(
    ...
    with create_and_store_resource(
        ...
        yield cast(NetworkMap, _network_map)
...
def create_storagemap_and_networkmap(
    ...
    automatically cleans them up when the context exits (unless skip_teardown=True).
    ...
    Args:
        ...
        skip_teardown: If True, skip cleanup on context exit

    Yields:
        tuple[StorageMap, NetworkMap]: The created storage and network maps

    Example:
        with create_storagemap_and_networkmap(..., skip_teardown=False) as (storage_map, network_map):
            # Use the maps
            pass
