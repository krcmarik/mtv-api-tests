...
def create_source_provider(
    ...
    # Prepare copy-offload annotation before branching
    ...
    if not source_provider_secret:
        # Non-OCP providers: Create Secret and keep context open
        with create_and_store_resource(
            ...
            with create_and_store_resource(
                ...
                ocp_resource_provider.wait_for_status(
                    Provider.Status.READY, timeout=600, stop_status="ConnectionFailed"
                )
        ...
        # OCP provider: Secret already exists (destination_ocp_secret), only nest Provider
        with create_and_store_resource(
            fixture_store=fixture_store,
            resource=Provider,
            client=admin_client,
...
def create_source_cnv_vms(
    ...
    for vm_dict in vms:
        # Use context manager with skip_teardown=True to track but not cleanup
        # These VMs must outlive this function and be cleaned up by session teardown
        # The context exits immediately after the loop iteration, but skip_teardown=True
        # prevents actual cleanup - resources remain tracked in fixture_store
        with create_and_store_resource(
...
""",
            ...
            skip_teardown=True,  # VMs must survive past this function for session use
        ) as vm:
            vms_to_create.append(vm)
